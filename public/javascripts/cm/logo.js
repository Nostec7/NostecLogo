  ...
  CodeMirror.defineMode('logo', function(config, parserConfig) {
    ...
    return {
      ...
      indent: function(state, textAfter) {
        ...
        case 'defn-vars':
        case 'defn-body':
          if ((/^END\b/i.test(textAfter)) || (/^TAŠKAS\b/i.test(textAfter)))
            return indent * size;
          return (indent + 1 ) * size;
        default:
          return indent * size;;
      ...
      token: function(stream, state) {
        ...
        if (state.state === 'normal') {
          if (stream.match(/^TO\b/i, true) || stream.match(/^TAI\b/i, true)) {
            state.state = 'defn-name';
            return 'logo-defn-start';
          }
          if (stream.match(/^END\b/i, true) || stream.match(/^TAŠKAS\b/i, true)) {
            return 'logo-error';
          }
        }
