...
(function(global) {
  ...
  function CanvasTurtle(canvas_ctx, turtle_ctx, w, h, events) {
    ...
    if (events) {
      ...
      var cursor = document.getElementById("display");
      events.addEventListener('mousedown', function(e) {
        var rect = events.getBoundingClientRect();



          if(Math.abs(this.y +((e.clientY - rect.top) - this.height/2)) <= 20 &&  //move turtle only if pressed on itself
              Math.abs(this.x +(-(e.clientX - rect.left) + this.width/2)) <= 20){
              isMouseDown = true; //start moving turtle
      ...
      events.addEventListener('mouseup', function(e) {
          isMouseDown = false; // release turtle
  ...
  Object.defineProperties(CanvasTurtle.prototype, {
    ...
    _init: {value: function() {
      ...
      this.turtle_ctx.strokeStyle = "#34995e";
    ...
    _tick: {value: function() { //turtle rendering radius
      ...
      if (this.visible) {
        ...
        ctx.closePath();
        ctx.fillStyle = "#34995e";
        ctx.fill();
        ctx.strokeStyle = "#2b7a4c";
        ctx.stroke();
    ...
    resize: {value: function(w, h) {
      //localStorage.setItem("canvas-resize-before", this.canvas_ctx.toDataURL());
      this.width = w;
      this.height = h;



        /*var dataURL = localStorage.getItem("canvas-resize-before");
    ...
    drawImage: {value: function(image) {
      //alert('hel');
      this.canvas_ctx.save();
      this.canvas_ctx.setTransform(1, 0, 0, 1, 0, 0);
